...
function AdminInvoice() {
  ...
  const {
    ...
    fetchingAPIInstance: ({ controller, page, take }) =>
      authInstance.get(
        [authConstraints.adminRoot, authConstraints.getAllPayments].join("/"),
        {
          headers: {
            Authorization: [
  ...
  function payout(id, hasPayout) {
    ...
    authInstance
      .put(
        [authConstraints.adminRoot, authConstraints.putPayoutPayments].join(
          "/"
        ),
        null,
        {
      ...
      .then((response) => {
        ...
        if (
          ...
          response.data?.result?.stripePaymentId
        ) {
          setItems((items) =>
            items.map((item) => {
              if (
                item?.stripePaymentId == response.data?.result?.stripePaymentId
              ) {
                            ...
                            .map((payment, index) => {
                              return (
                                <tr key={index}>
                                  ...
                                  <td>
                                    {payment?.completedAt
                                      ? moment(payment?.completedAt).format(
                                          "YYYY/MM/DD"
                                        )
                                      : "null"}
                                  </td>
                                  <td>
                                    {!payment?.hasPayout ? (
                                      ...
                                      <span className="content-green">
                                        {moment(payment?.payoutAt).format(
                                          "HH:mm YYYY/MM/DD"
                                        )}
                                      </span>
                                    )}
...
export default function Index() {
  return <AdminInvoice></AdminInvoice>;
}
